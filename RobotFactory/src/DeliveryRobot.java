...
public class DeliveryRobot implements Robot {
    ...
    public Status[][] doWork(int room, int item, Status[][] roomArray) {
        ...
        // 並判別這塊正方形區域是否與其他貨物重疊或是空間不足擺放貨物，若是則重複此步驟，若否則進行下一步驟
        ...
        // 由送貨機器人工廠生產一個送貨機器人擺放貨物，並將貨物擺放範圍設為 Status.GOODS (已擺放貨物)
        for (int y = col; y <= (col+item-1); y++) {
            for (int x = row; x <= (row+item-1); x++) {
                roomArray[x][y] = Status.GOODS;
            }
        }
